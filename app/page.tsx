...
export default async function PublicKatalogPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F0F2F5] font-sans selection:bg-emerald-200">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-[72px] flex items-center justify-between gap-4">
          ...
          <Link href="/" className="flex items-center gap-3 shrink-0">
            <div className="relative w-10 h-10 flex-shrink-0">
              ...
              />
              {/* Fallback icon jika gambar gagal load (ada di background) */}
              <div className="absolute inset-0 bg-gradient-to-br from-[#1B4332] to-emerald-600 rounded-xl flex items-center justify-center -z-10">
          ...
          <div className="flex items-center gap-2">
            {isLoggedIn ? (
              <Link href="/dashboard" className="hidden sm:flex items-center gap-2 px-5 py-2.5 bg-[#1B4332] text-white rounded-xl text-[11px] font-bold uppercase tracking-wider hover:bg-[#143628] transition-all shadow-md shadow-emerald-900/20 shrink-0">
                <span>🛡️</span> Dashboard
              </Link>
            ) : (
              <Link href="/login" className="hidden sm:flex items-center gap-2 px-5 py-2.5 border-2 border-[#1B4332] text-[#1B4332] rounded-xl text-[11px] font-bold uppercase tracking-wider hover:bg-emerald-50 transition-all shrink-0">
                <span>🔐</span> Login
              </Link>
            )}
      ...
      {/* ── HERO ── */}
      <header className="relative bg-[#0f2a1c] overflow-hidden pt-20 sm:pt-24 pb-32 sm:pb-36">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
          <div className="max-w-3xl">
            <div className="anim-up-1 inline-flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-900/60 border border-emerald-700/50 text-emerald-300 text-[10px] font-bold uppercase tracking-[0.18em] mb-6 sm:mb-8 backdrop-blur-sm">
              <span className="w-1.5 h-1.5 bg-emerald-400 rounded-full animate-pulse" />
              Pusat Literasi & Referensi Hukum
            </div>
            <h2 className="anim-up-2 font-display text-[2rem] sm:text-[2.6rem] md:text-[3.6rem] text-white leading-[1.1] mb-5 sm:mb-6">
              Menjaga Lentera{' '}
              <span className="shimmer-text">Keadilan</span>
              <br />
              Lewat Pengetahuan.
            </h2>
            <p className="anim-up-3 text-emerald-100/70 font-medium max-w-xl text-[14px] sm:text-[15px] leading-relaxed mb-8 sm:mb-10">
            ...
            <div className="anim-up-4 flex flex-col sm:flex-row gap-3 sm:gap-4">
              <Link href="#katalog" className="inline-flex items-center justify-center gap-2 px-6 sm:px-8 py-3.5 sm:py-4 bg-amber-400 hover:bg-amber-300 text-[#1B4332] rounded-2xl text-[12px] font-black uppercase tracking-widest shadow-[0_0_32px_rgba(251,191,36,0.35)] hover:shadow-[0_0_48px_rgba(251,191,36,0.45)] hover:-translate-y-1 transition-all duration-300">
              ...
              <Link href="#layanan" className="inline-flex items-center justify-center gap-2 px-6 sm:px-8 py-3.5 sm:py-4 bg-white/8 hover:bg-white/14 text-white border border-white/20 rounded-2xl text-[12px] font-bold uppercase tracking-widest backdrop-blur-md transition-all duration-300">
                Info Layanan
              </Link>
            </div>
          </div>
      ...
      {/* ── STATS BANNER ── */}
      <div className="max-w-5xl mx-auto px-4 sm:px-6 relative z-20 -mt-14 sm:-mt-16 mb-16 sm:mb-20">
        <div className="bg-white rounded-[1.5rem] sm:rounded-[1.75rem] shadow-[0_20px_60px_rgba(0,0,0,0.08)] border border-slate-100 overflow-hidden">
          <div className="grid grid-cols-3 divide-x divide-slate-100">
            {[
              ...
              <div key={s.label} className="p-4 sm:p-6 md:p-8 flex flex-col md:flex-row items-center md:items-start gap-2 sm:gap-3 text-center md:text-left">
                <span className="text-xl sm:text-2xl md:text-3xl mt-0.5 hidden sm:block">{s.icon}</span>
                <div>
                  <p className="text-xl sm:text-2xl md:text-3xl font-black text-slate-900 font-display leading-none">{s.value}</p>
                  <p className="text-[9px] sm:text-[10px] font-semibold text-slate-400 uppercase tracking-widest mt-1">{s.label}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* ── LAYANAN ── */}
      <section id="layanan" className="py-12 sm:py-16 max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex flex-col items-center text-center mb-10 sm:mb-12">
          ...
          <h3 className="font-display text-2xl sm:text-3xl md:text-4xl text-slate-900">Fasilitas & Layanan</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-5">
          {[
            { icon: '📱', color: 'emerald', title: 'Sistem QR Terpadu', desc: 'Scan QR Code pada fisik buku untuk melihat detail, histori, dan ketersediaan stok secara real-time langsung dari kamera HP Anda.' },
            { icon: '📚', color: 'amber', title: 'Ruang Baca Publik', desc: 'Tersedia ruang baca yang nyaman di area pelayanan terpadu untuk masyarakat yang ingin mencari referensi hukum di tempat.' },
            { icon: '🤝', color: 'blue', title: 'Peminjaman Internal', desc: 'Khusus pegawai kejaksaan terdaftar dapat melakukan peminjaman buku/aset secara digital untuk keperluan dinas.' },
          ].map((s) => (
            <div key={s.title} className="group bg-white rounded-[1.5rem] p-6 sm:p-8 border border-slate-100 card-hover relative overflow-hidden">
              <div className={`absolute top-0 right-0 w-20 h-20 rounded-bl-[3rem] opacity-0 group-hover:opacity-100 transition-opacity duration-500 ${s.color === 'emerald' ? 'bg-emerald-50' : s.color === 'amber' ? 'bg-amber-50' : 'bg-blue-50'}`} />
              <div className={`w-12 sm:w-14 h-12 sm:h-14 rounded-2xl flex items-center justify-center text-xl sm:text-2xl mb-5 sm:mb-6 ${s.color === 'emerald' ? 'bg-emerald-50' : s.color === 'amber' ? 'bg-amber-50' : 'bg-blue-50'} group-hover:scale-110 transition-transform duration-300`}>
                {s.icon}
              </div>
              <h4 className="font-bold text-slate-800 text-[15px] sm:text-[16px] mb-2 sm:mb-3 leading-snug">{s.title}</h4>
              <p className="text-[12px] sm:text-[13px] text-slate-500 leading-relaxed">{s.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* ── KATALOG ── */}
      <main id="katalog" className="py-16 sm:py-20 bg-gradient-to-b from-slate-100/60 to-slate-100 border-t border-slate-200/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-8 sm:mb-10 gap-4">
            <div>
              ...
              <h3 className="font-display text-2xl sm:text-3xl md:text-4xl text-slate-900">Katalog Literatur Hukum</h3>
      ...
      <footer className="bg-[#0a1f12] text-slate-400">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12 sm:py-16 grid grid-cols-1 md:grid-cols-3 gap-8 sm:gap-10">
          <div>
            <div className="flex items-center gap-3 mb-4">
              <div className="w-9 h-9 relative flex-shrink-0">
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-br from-emerald-500 to-[#1B4332] rounded-xl -z-10" />
              </div>
          ...
          <div>
            ...
            <div className="space-y-2 text-[12px]">
              <div className="flex justify-between border-b border-white/5 pb-2"><span className="text-slate-400">Senin – Jumat</span><span className="text-white font-semibold">08:00 – 16:00</span></div>
              <div className="flex justify-between border-b border-white/5 pb-2"><span className="text-slate-400">Sabtu – Minggu</span><span className="text-red-400 font-semibold">Libur</span></div>
              <div className="flex justify-between pt-1"><span className="text-slate-400">Akses Digital</span><span className="text-emerald-400 font-semibold">24 Jam</span></div>
            </div>
          </div>
        </div>
