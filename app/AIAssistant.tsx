  ...
  const renderFormattedText = (text: string) => {
    ...
    return parts.map((part, index) => {
      if (part.startsWith('**') && part.endsWith('**')) {
        return <strong key={index} className="font-black text-emerald-800">{part.slice(2, -2)}</strong>;
      }
      ...
      {isOpen && (
        <>
          ...
          `}>
            ...
            {/* HEADER */}
            <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-[#1B4332] p-4 sm:p-5 flex justify-between items-center relative overflow-hidden shrink-0">
              ...
              <div className="flex items-center gap-3 relative z-10">
                <div className="w-10 sm:w-12 h-10 sm:h-12 bg-gradient-to-tr from-emerald-400 to-emerald-600 rounded-xl sm:rounded-2xl flex items-center justify-center shadow-lg border border-white/20 relative">
                  <span className="text-xl sm:text-2xl">🤖</span>
                  <span className="absolute -bottom-1 -right-1 w-3 sm:w-3.5 h-3 sm:h-3.5 bg-emerald-400 border-2 border-slate-800 rounded-full animate-pulse" />
                </div>
                <div>
                  <h3 className="font-black text-white text-[13px] sm:text-[15px] tracking-tight leading-tight">Lexi — Asisten Perpustakaan</h3>
                  <p className="text-[9px] sm:text-[10px] font-bold text-emerald-400 uppercase tracking-widest mt-0.5 opacity-90 hidden sm:block">Kejaksaan Negeri Soppeng · Powered by AI</p>
                </div>
              </div>
            ...
            <div className="p-3 sm:p-4 bg-white border-t border-slate-100 shrink-0">
              ...
              >
                <input
                  ...
                  className="flex-1 bg-slate-50 border border-slate-200 rounded-2xl py-3 sm:py-4 pl-4 sm:pl-5 pr-12 sm:pr-14 text-sm font-semibold text-slate-800 focus:outline-none focus:border-emerald-500 focus:bg-white focus:ring-4 focus:ring-emerald-500/10 transition-all placeholder:font-medium placeholder:text-slate-400"
                  value={input}
                ...
                <button
                  ...
                  className="absolute right-2 w-9 sm:w-10 h-9 sm:h-10 bg-gradient-to-br from-emerald-500 to-[#1B4332] text-white rounded-[12px] flex items-center justify-center hover:scale-105 disabled:bg-none disabled:bg-slate-200 disabled:text-slate-400 disabled:scale-100 transition-all shadow-md disabled:shadow-none"
                >
      ...
      >
        {isOpen ? (
          ...
          <div className="relative">
            ...
            <span className="absolute -top-1 -right-2 w-3 sm:w-3.5 h-3 sm:h-3.5 bg-rose-500 border-2 border-[#1B4332] rounded-full animate-ping" />
            <span className="absolute -top-1 -right-2 w-3 sm:w-3.5 h-3 sm:h-3.5 bg-rose-500 border-2 border-[#1B4332] rounded-full" />
          </div>
        )}
      </button>
