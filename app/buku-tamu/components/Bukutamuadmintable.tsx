...
// ── Kartu entri ───────────────────────────────────────────────
function EntryCard({ entry, no }: { entry: BukuTamuEntry; no: number }) {
  ...
  const cfg = statusConfig[entry.status] ?? { cls: 'bg-slate-50 text-slate-500 border-slate-200', label: entry.status };

  return (
    <>
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow p-4 flex flex-col gap-3 relative overflow-hidden group">
        {/* Nomor badge */}
        ...
        <div>
          ...
          <p className="text-[10px] text-slate-400 font-bold mt-0.5">{formatTanggal(entry.created_at)}</p>
        </div>
        ...
        {/* Pesan / Kritik */}
        {entry.pesan && (
          <p className="text-[11px] text-slate-500 font-medium italic bg-slate-50 rounded-xl px-3 py-2 border border-slate-100 line-clamp-2">
        ...
        {entry.ttd_data && (
          <button onClick={() => setShowTtd(true)} className="self-start group/ttd">
            ...
            />
            <p className="text-[8px] font-bold text-emerald-600 mt-0.5 opacity-0 group-hover/ttd:opacity-100 transition-opacity">Perbesar ↗</p>
          </button>
        )}
        ...
        <div className="flex gap-1.5 mt-auto pt-2 border-t border-slate-50">
          {entry.status !== 'approved' && (
            <button disabled={loading} onClick={handleApprove}
              className="flex-1 py-2 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-emerald-100 transition-colors disabled:opacity-50">
              ACC
            </button>
          )}
          {entry.status !== 'rejected' && (
            <button disabled={loading} onClick={handleReject}
              className="flex-1 py-2 bg-amber-50 text-amber-700 border border-amber-200 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-amber-100 transition-colors disabled:opacity-50">
              Tolak
            </button>
          )}
          <button disabled={loading} onClick={() => setShowDel(true)}
            className="px-3 py-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-all border border-transparent hover:border-red-100"
            title="Hapus">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              ...
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">
                {filtered.length} dari {safeEntries.length} entri ditampilkan
              </p>
            </div>
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {filtered.map((entry, i) => (
              <EntryCard key={entry.id} entry={entry} no={i + 1} />
            ))}
          </div>
